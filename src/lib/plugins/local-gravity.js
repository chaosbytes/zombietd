 ...
 */
ig.module('plugins.local-gravity').requires('impact.entity').defines(function () {
	...
	ig.Entity.inject({
		...
		update: function () {
			...
			if (this.localGravity.mass) {
				...
				for (var i = 0; i < ents.length; i++) {
					...
					if (ent == this || !ent.localGravity.mass) continue;
		...
		gravitateWith: function (other) {
			...
			if (distSQ > 0.0001) {
				...
				if (this.localGravity.range && dist > this.localGravity.range) return;
