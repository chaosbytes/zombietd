ig.module('plugins.Camera').requires('impact.impact').defines(function () {
	Camera = ig.Class.extend({
		...
		move: function (axis, pos, size) {
			...
			return (
			this.pos[axis] - (
			this.pos[axis] - this.trap.pos[axis] + this.offset[axis] + this.currentLookAhead[axis]) * ig.system.tick * this.damping).limit(0, this.max[axis]);
		},
